...
3. **Gemini inlineData/fileData 不支持**: 图像/二进制 parts 当前跳过
4. **生产尚未推送最新 Gemini 端点**: 本地已验证但生产代码仍是手动更新版（含 reasoning/错误码修复，不含 Gemini/nano_banana_2 补齐），需推送 `754d1a9` 一起上线

## 下一步（按优先级）

1. **推送上线** → `754d1a9` 含 Gemini 端点 + nano_banana_2 补齐
2. **Anthropic 格式端口** → marker `anthropic` 已预留，TDD 流程
3. **Tools / Function Calling** → marker `tools` 已预留，需先探针确认上游能力
4. **Vision 输入** → 需进一步实验 sider 插件真实图像格式
